...
`,Rt=()=>{const[n,e]=d.useState(void 0),i=At(n),{nextWeek:t,previousWeek:s,startWeekWithToday:a}=Lt(e);d.useEffect(()=>{a()},[]);const o=Ot({onSwipedLeft:()=>t(),onSwipedRight:()=>s()}),c=m.useMemo(()=>({weekday:"short",month:"numeric",day:"numeric"}),[]),u=m.useMemo(()=>i.slice(0,7),[i]);return r.jsxs(It,{...o,children:[r.jsx(vt,{nextWeek:t,previousWeek:s,startWeekWithToday:a}),r.jsxs("div",{className:"schedule",children:[u.map((l,f)=>r.jsx("div",{className:j({weekend:G(l.date),today:F(l.date)},"caption"),children:r.jsx("h2",{children:l.date.toLocaleString(c)})},f)),u.map((l,f)=>r.jsx("div",{className:j("allDayRow",{weekend:G(l.date),today:F(l.date)}),children:l.allDay.map((h,p)=>r.jsx("div",{className:"allDayEvent",children:h.summary},p))},f)),u.map((l,f)=>r.jsx("div",{className:j("eventRow",{weekend:G(l.date),today:F(l.date)}),children:l.events.map((h,p)=>r.jsxs("div",{className:"event",children:[r.jsx("div",{children:h.summary}),r.jsxs("h3",{children:[h.icon&&r.jsx(_,{path:h.icon,size:"1rem",color:"#ffffff"}),ie(h.start.dateTime)," - ",ie(h.end.dateTime)]})]},p))},f))]}),i.length===0&&r.jsx("div",{className:"loading",children:r.jsx(de,{visible:!0,height:"80",width:"80",color:"#c1c1c1",radius:"9",ariaLabel:"three-dots-loading",wrapperStyle:{},wrapperClass:""})})]})},M={"clear-day":{icon:ke,label:"Klar",color:"#eeeef5"},"clear-night":{icon:De,label:"Klar",color:"#eeeef5"},rain:{icon:_e,label:"Regen",color:"#80a5d6"},snow:{icon:Te,label:"Schnee",color:"#8c82ce"},sleet:{icon:Se,label:"Graupel",color:"#aba4db"},wind:{icon:je,label:"Stürmisch",color:"#9fb6d6"},fog:{icon:ve,label:"Neblig",color:"#d5dae2"},cloudy:{icon:Ee,label:"Bewölkt",color:"#b6bfcb"},"partly-cloudy-day":{icon:be,label:"Teils bewölkt",color:"#d5dae2"},"partly-cloudy-night":{icon:we,label:"Teils bewölkt",color:"#d5dae2"}},Wt=()=>`./forecast/${J}/${ue},${fe}?&units=si&exclude=minutely`,Pt=n=>{const[e,i]=d.useState([]),t=X(6e4*10,"Weather"),s=nt&&J&&ue&&fe;return d.useEffect(()=>{s&&y(Wt()).then(a=>{i(a.data)}).catch(a=>{b.error("Could not load weather data",a)}).finally(()=>{})},[t,n,s]),e},Mt=Ne(Ae),Ht=w.div`
